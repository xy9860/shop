...
$(function () { 
	...
	$('#dg').datagrid({
	    url:'category_queryJoinAccount.action',//请求数据的url地址
	    queryParams:{ctype:''},//当请求远程数据时，发送的额外参数。
	    idField:'cid',//指定cid为标识字段 可以隔着页面选中
	    striped:true,//奇偶行变色
	    ...
	    frozenColumns:[[//冻结这列 不能移动
	    	 ...
	    	 {field:'cid',title:'类别编号',width:100}
	    ]],
	    ...
	    columns:[[//配置列字段    field是列字段名称与json key捆绑 tietle 标题 是显示层

	        {field:'ctype',title:'类别名称',width:100,formatter: function(value,row,index){
				//用来格式化当前列的值 返回当前列的数据
				return "<span title="+value+">" +value+"</span>";//鼠标放上去提示当前内容
			}
},
	        {field:'chot',title:'推荐',width:100,align:'right',formatter: function(value,row,index){
				//用来格式化当前列的值 返回当前列的数据
				if(value){
				...
				if (value <= 12){//返回符合条件的列的 style
					return 'background-color:#ffee00;color:red;';
				}
			} */
		},
		{field:'account.alogin',title:'所属管理员',width:100,formatter: function(value,row,index){//这里不能直接用列名读取 必须使用格式输出
				//用来格式化当前列的值 返回当前列的数据
				 if(row.account!=null&&row.account.alogin!=null){
					return row.account.alogin;
				}else{
					return "暂无";
				}
			}
				...
				if(rows.length==0){
					...
					$.messager.confirm('确认执行', '确认删除选中的记录吗?', function(r){	
						 if (r) {
			               ...
			               $.post("category_deleteByIds.action",{ids:ids},function (result){//ajax 提交
			            	   //回调函数
